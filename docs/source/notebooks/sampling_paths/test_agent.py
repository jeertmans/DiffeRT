...
class TestAgent:
    ...
    def test_train(
        ...
        if degenerate:
            ...
            def random_scene(key: PRNGKeyArray) -> TriangleScene:
                return eqx.tree_at(
                    lambda x: x.mesh, scene, scene.mesh.shuffle(key=key)
                )
        ...
        if not paths.mask.all():  # type: ignore[possibly-missing-attribute]
            # agent = eqx.tree_at(lambda a: a.model.debug, agent, True)
            # path_candidates = jax.lax.map(
            #     lambda key: agent.model(scene, inference=True, key=key),
            #     jr.split(eval_key, 100),
            # )
            invalid_paths = path_candidates[~paths.mask, :]  # type: ignore[unsupported-operator]
            per_invalid = (
                100 * invalid_paths.shape[0] / path_candidates.shape[0]
            )
            msg = f"Agent trained model (final loss: {loss:.2e}) but generated path candidates still contain invalid paths ({per_invalid:.2f}% of them):\n{invalid_paths}, but valid are:\n{valid_paths.objects[:, 1:-1]}"
