...
class Memory(eqx.Module):
    ...
    def __init__(
        self, memory_size: int, order: int, key_dtype: jnp.dtype
    ) -> None:
    ...
    @eqx.filter_jit
    def add_successful_experiences(
        self,
        ...
        sort_idx = jnp.argsort(rewards, descending=True)
