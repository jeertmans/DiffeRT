...
class TriangleScene(eqx.Module):
        ...
        Args:
            ...
            solver: The solver configuration or string shortcut.
                ...
                * If ``'hybrid'``, a hybrid method is used, which estimates the objects
                  ...
                  bounces or the size of the scene.
            ...
            **solver_kwargs: Parameters to override in the solver configuration:
                ...
                * **chunk_size** (:class:`int` | :data:`None`): If specified, iterates through chunks
                  of path candidates, yielding an iterator over path chunks.
                * **epsilon** (:class:`float` | :data:`None`): Tolerance for checking ray / object intersections.
                * **hit_tol** (:class:`float` | :data:`None`): Tolerance for blockage checks.
                * **min_len** (:class:`float` | :data:`None`): Minimal squared length of path segments.
                * **smoothing_factor** (:class:`float` | :data:`None`): Parameters for slope of the smoothing function.
                * **confidence_threshold** (:class:`float`): Confidence threshold for valid paths.
                * **batch_size** (:class:`int` | :data:`None`): Intersection check batch size.
                * **disconnect_inactive_triangles** (:class:`bool`): Whether to filter out inactive triangles first.
    ...
    def launch_paths(
        ...
        **solver_kwargs: Any,
        ...
        Args:
            ...
            solver: The solver configuration or string shortcut (e.g. ``'sbr'``).
            **solver_kwargs: Parameters to override in the solver configuration:

                * **num_rays** (:class:`int`): The number of rays launched.
                * **max_dist** (:class:`float`): Maximal (squared) distance between a receiver and a ray
                  for the receiver to be considered in the vicinity of the ray path.
    ...
    def compute_paths(
        ...
        *,
        ...
        if method in {"sbr", "hybrid"} and order is None:
            msg = f"Argument 'order' is required when 'method == \"{method}\"'."
            raise ValueError(msg)
        ...
        if method == "sbr":
            ...
            return self.launch_paths(order=cast("int", order), solver=solver)
        if method == "hybrid":
            solver = HybridPathSolver(
                num_rays=num_rays,
                epsilon=epsilon,
