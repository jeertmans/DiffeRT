...
from .utils import geometric_transformation, unpack_scene, random_scene
    ...
    def test_scene_encoder(
        ...
        for i, subtest_key in enumerate(jr.split(key, 10)):
            with subtests.test(i=i):
                ...
                ref_objects_embeds = objects_encoder(ref_xyz, active_objects=ref_scene.mesh.mask)
                ref_scene_embeds = scene_encoder(ref_objects_embeds, active_objects=ref_scene.mesh.mask)

                for shuffle_key in jr.split(shuffles_key, 3):
                    scene = eqx.tree_at(lambda s: s.mesh, ref_scene, ref_scene.mesh.shuffle(key=shuffle_key))
    ...
    def test_state_encoder(
        ...
        for i, subtest_key in enumerate(jr.split(key, 10)):
            with subtests.test(i=i):
                ...
                path_candidate = jr.choice(path_candidate_key, ref_xyz.shape[0], (order,), replace=False, p=p)

                for i in range(order):
                    ref_partial_path_candidate = path_candidate.at[i:].set(-1)

                    ref_state_embeds = state_encoder(ref_partial_path_candidate, objects_embeds=ref_objects_embeds, active_objects=ref_scene.mesh.mask)

                    for shuffle_key in jr.split(shuffles_key, 3):
                        mesh, indices = ref_scene.mesh.shuffle(return_indices=True, key=shuffle_key)
                        rev_indices = jnp.empty_like(indices)
                        rev_indices = rev_indices.at[indices].set(jnp.arange(num_objects))
                        partial_path_candidate = rev_indices.at[ref_partial_path_candidate].get(wrap_negative_indices=False, mode="fill", fill_value=-1)
                        scene = eqx.tree_at(lambda s: s.mesh, ref_scene, mesh)
                        xyz = geometric_transformation(*unpack_scene(scene))
                        objects_embeds = objects_encoder(xyz, active_objects=scene.mesh.mask)
                        state_embeds = state_encoder(partial_path_candidate, objects_embeds=objects_embeds, active_objects=scene.mesh.mask)
