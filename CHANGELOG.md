...
- Generalized `tx_wavefront_radius` (on {meth}`GeometricFieldSolver.compute_fields<differt.em.GeometricFieldSolver.compute_fields>`, {meth}`GeometricFieldSolver.spreading_factor<differt.em.GeometricFieldSolver.spreading_factor>`, and {func}`compute_received_fields<differt.em.compute_received_fields>`) to accept a `(rho_s, rho_p)` tuple of two independent principal radii of curvature, for a general **astigmatic** (as opposed to spherical) non-planar source, or `None`, for a **planar** wavefront (the far-field, plane-wave approximation, e.g. via {class}`FarFieldAntenna<differt.em.FarFieldAntenna>`); a single value still defaults to an isotropic (spherical) wavefront. An astigmatic source (unlike a planar one) is not yet supported together with a `DIFFRACTION` interaction, which would require tracking the wavefront's principal-axis orientation along the path (by <gh-user:jeertmans>, in <gh-pr:460>).
- Added {meth}`Antenna.wavefront_radii<differt.em.Antenna.wavefront_radii>`, letting each antenna report the radius (or radii) of curvature of the wavefront it emits, at a given observation point, in the local s-/p-plane basis; {meth}`GeometricFieldSolver.compute_fields<differt.em.GeometricFieldSolver.compute_fields>` uses it (overriding `tx_wavefront_radius`) whenever `tx_polarization` is an {class}`Antenna<differt.em.Antenna>` instance. {class}`Dipole<differt.em.Dipole>` implements it as a spherical wavefront centered on its own {attr}`~differt.em.Antenna.center`. Added {class}`FarFieldAntenna<differt.em.FarFieldAntenna>`, a base class for antennas only ever used in the far field (`wavefront_radii` unconditionally returns `None`, i.e., a planar wavefront), and {class}`FarFieldDipoleAntenna<differt.em.FarFieldDipoleAntenna>`, the corresponding far-field-only variant of {class}`Dipole<differt.em.Dipole>` (by <gh-user:jeertmans>, in <gh-pr:460>).
