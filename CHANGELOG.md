...
- Fixed {class}`MaterialsDict<differt.em._material.MaterialsDict>` being unhashable, which made {attr}`GeometricFieldSolver.radio_materials<differt.em.GeometricFieldSolver.radio_materials>` (or the new {attr}`Scene.radio_materials<differt.geometry.Scene.radio_materials>`) raise a `TypeError` as soon as a JIT-compiled function was called with a non-default value set (by <gh-user:jeertmans>, in <gh-pr:537>).
