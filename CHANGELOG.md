...
- Added topology-aware diffraction edge detection properties (`_connectivity`, `diffraction_edges_mask`, `diffraction_edges`, `wedge_angles`, `wedge_parameters`) on {class}`Mesh<differt.geometry.Mesh>` to support edge adjacency, quad diagonal exclusion, non-manifold edge warnings, and convex/concave/knife-edge wedge angle classification (by <gh-user:jeertmans>, in <gh-pr:460>).
- Added a reusable, JAX-compatible EM pipeline ({func}`compute_received_fields<differt.em.compute_received_fields>`, {func}`compute_received_power<differt.em.compute_received_power>`, {func}`compute_cir<differt.em.compute_cir>`) under {mod}`differt.em` to calculate received fields, received power, and Channel Impulse Response (CIR) (by <gh-user:jeertmans>, in <gh-pr:460>).
- Added comparative integration tests verifying DiffeRT's computed received power against Sionna's RT solver (by <gh-user:jeertmans>, in <gh-pr:460>).
- Added UTD {func}`diffraction_coefficients<differt.em.diffraction_coefficients>` implementing soft ($D_s$) and hard ($D_h$) diffraction coefficients for PEC and lossy dielectric wedges (Luebbers' heuristic model), including a NaN-safe transition helper across shadow and reflection boundaries (by <gh-user:jeertmans>, in <gh-pr:460>).
- Added {class}`AbstractFieldSolver<differt.em.AbstractFieldSolver>` and {class}`GeometricFieldSolver<differt.em.GeometricFieldSolver>`, generalizing {func}`compute_received_fields<differt.em.compute_received_fields>` into an extensible, subclassable field-solver architecture (mirroring the {class}`AbstractPathSolver<differt.geometry.AbstractPathSolver>` hierarchy on the path tracing side), so custom EM interaction models can be added by overriding a `*_matrix` method and declaring it in `supported_interaction_types`, without modifying the pipeline itself (by <gh-user:jeertmans>, in <gh-pr:460>).
- Added {attr}`InteractionType.TRANSMISSION<differt.em.InteractionType.TRANSMISSION>` (by <gh-user:jeertmans>, in <gh-pr:460>).
- Implemented {meth}`GeometricFieldSolver.transmission_matrix<differt.em.GeometricFieldSolver.transmission_matrix>` (a finite-thickness dielectric slab model, per ITU-R P.2040-3 eqs. 43-44, matching Sionna RT's transmission model) and {func}`slab_coefficients<differt.em.slab_coefficients>`, the shared reflection/transmission slab formula it (and {meth}`GeometricFieldSolver.reflection_matrix<differt.em.GeometricFieldSolver.reflection_matrix>`) is built on (by <gh-user:jeertmans>, in <gh-pr:460>).
- Implemented {meth}`GeometricFieldSolver.diffraction_matrix<differt.em.GeometricFieldSolver.diffraction_matrix>`, wiring the Uniform Theory of Diffraction ({func}`diffraction_coefficients<differt.em.diffraction_coefficients>`) to a mesh's wedge geometry ({attr}`Mesh.wedge_angles<differt.geometry.Mesh.wedge_angles>` and half-edge adjacency), including the same face-orientation and shadow-boundary conventions as Sionna RT's dielectric-wedge model; validated to match Sionna RT's per-path channel coefficient to 5+ significant figures on a canonical 90° wedge scene (by <gh-user:jeertmans>, in <gh-pr:460>).
- Implemented {meth}`GeometricFieldSolver.scattering_matrix<differt.em.GeometricFieldSolver.scattering_matrix>`, a Lambertian rough-surface diffuse scattering model (Degli-Esposti's directive model, as also used by Sionna RT), together with new {attr}`Material.scattering_coefficient<differt.em.Material.scattering_coefficient>` and {attr}`Material.xpd_coefficient<differt.em.Material.xpd_coefficient>` fields (both defaulting to `0.0`, so existing materials are unaffected); this is a deterministic adaptation of Sionna RT's inherently Monte Carlo model and is not expected to numerically match it for a single path (by <gh-user:jeertmans>, in <gh-pr:460>).
- `GeometricFieldSolver`'s default `supported_interaction_types` now includes all four {class}`InteractionType` members (previously reflection only), reflecting the above (by <gh-user:jeertmans>, in <gh-pr:460>).
- Added a `tx_wavefront_radius` argument to {func}`compute_received_fields<differt.em.compute_received_fields>` and {meth}`GeometricFieldSolver.compute_fields<differt.em.GeometricFieldSolver.compute_fields>` (and the ``*_matrix``/`spreading_factor`/`transition_matrices` methods it flows through) to support a non-planar (near-field) source, defaulting to `0` (an ideal point source, unchanged behavior); unlike the reflection/transmission/diffraction/scattering models above, Sionna RT has no equivalent feature to validate against, so correctness is instead checked against the equivalent of physically moving the transmitter back by `tx_wavefront_radius` (by <gh-user:jeertmans>, in <gh-pr:460>).

### Fixed

- Fixed {func}`diffraction_coefficients<differt.em.diffraction_coefficients>` using the incidence angle instead of the diffraction angle when computing the n-face reflection coefficient (`cos_theta_n`), a pre-existing bug caught while cross-validating the new {meth}`GeometricFieldSolver.diffraction_matrix<differt.em.GeometricFieldSolver.diffraction_matrix>` against Sionna RT; only affected non-PEC (`n_r_n` provided) wedge faces (by <gh-user:jeertmans>, in <gh-pr:460>).
