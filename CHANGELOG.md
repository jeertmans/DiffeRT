...
- Added ergonomic aliases for {class}`InteractionType<differt.em.InteractionType>` members: {data}`SpecularReflection<differt.em.SpecularReflection>`, {data}`Diffraction<differt.em.Diffraction>`, {data}`Scattering<differt.em.Scattering>`, {data}`Transmission<differt.em.Transmission>`, and {data}`RIS<differt.em.RIS>` (by <gh-user:jeertmans>, in <gh-pr:537>).
- Key classes, functions, and interaction types ({class}`Scene<differt.geometry.Scene>`, {class}`Mesh<differt.geometry.Mesh>`, {class}`TracedPaths<differt.geometry.TracedPaths>`, {class}`LaunchedPaths<differt.geometry.LaunchedPaths>`, {class}`Material<differt.em._material.Material>`, {class}`InteractionType<differt.em.InteractionType>`, {data}`SpecularReflection<differt.em.SpecularReflection>`, {data}`Diffraction<differt.em.Diffraction>`, {data}`Scattering<differt.em.Scattering>`, {data}`Transmission<differt.em.Transmission>`, {data}`RIS<differt.em.RIS>`, {class}`WavefrontState<differt.em.WavefrontState>`, and {func}`propagate_wavefront<differt.em.propagate_wavefront>`) are now lazily re-exported directly from the top-level `differt` package (e.g., `differt.Scene`), fully documented in the {doc}`top-level package reference <reference/differt>`, without making `import differt` any more expensive, since the defining submodule is only imported on first access of one of these names (by <gh-user:jeertmans>, in <gh-pr:537>).
- {meth}`Scene.trace_paths<differt.geometry.Scene.trace_paths>`, {meth}`Scene.trace_fields<differt.geometry.Scene.trace_fields>`, and {meth}`AbstractPathTracer.generate_path_candidates<differt.geometry.AbstractPathTracer.generate_path_candidates>` now accept an `allowed_interactions` keyword argument (a `frozenset` of {class}`InteractionType<differt.em.InteractionType>`, e.g. `frozenset({SpecularReflection, Diffraction})`), replacing the previous, effectively-unreachable `specular_reflection`/`diffuse_scattering` booleans. {class}`ExhaustivePathTracer<differt.geometry.ExhaustivePathTracer>`, {class}`HybridPathTracer<differt.geometry.HybridPathTracer>`, and {class}`SBRPathTracer<differt.geometry.SBRPathTracer>` can now generate and trace ``DIFFRACTION`` (via {func}`fermat_path_on_linear_objects<differt.geometry.fermat_path_on_linear_objects>`, mixing reflection/scattering planes and diffraction edges in one joint solve) and ``TRANSMISSION`` (which does not bend the ray: its position is spliced in afterward, as the intersection of the straight segment between its two neighboring solved bounces with its transmissive face) bounces, in addition to today's ``REFLECTION``/``SCATTERING``. Defaults to reflection-only, matching prior behavior exactly (by <gh-user:jeertmans>, in <gh-pr:537>).
- Added {class}`WavefrontState<differt.em.WavefrontState>`, {class}`PathWavefront<differt.em.PathWavefront>`, and {func}`propagate_wavefront<differt.em.propagate_wavefront>` to support near-field wavefront curvature tracking and transport through free space, planar reflections, transmissions, and straight-edge diffractions. {class}`GeometricFieldSolver<differt.em.GeometricFieldSolver>` now supports astigmatic incident wavefronts with ``DIFFRACTION`` interactions via UTD astigmatic curvature formulas, lifting the prior runtime restriction. In addition, {class}`GeometricFieldSolver<differt.em.GeometricFieldSolver>` accepts custom pluggable interaction matrices via `interaction_matrices` (e.g., for custom RIS devices or user-defined interaction types) (by <gh-user:jeertmans>, in <gh-pr:537>).
- Added an integration test suite ({file}`differt/tests/integration/test_against_mcnamara_utd_book.py`) validating geometrical optics reflection and uniform theory of diffraction (UTD) formulas against 40 canonical equations from D. A. McNamara et al. (1990) *Introduction to the Uniform Geometrical Theory of Diffraction*, covering ray-fixed polarization, PEC reflection dyadics, line source reflection, principal wavefront curvatures, Keller's law of edge diffraction, edge-fixed coordinate systems, transition functions, astigmatic distance parameters, and caustic spreading factors (by <gh-user:jeertmans>, in <gh-pr:537>).
- Added a `max_diffractions` parameter to {class}`ExhaustivePathTracer<differt.geometry.ExhaustivePathTracer>`, {class}`HybridPathTracer<differt.geometry.HybridPathTracer>`, and {class}`SBRPathTracer<differt.geometry.SBRPathTracer>` to cap the maximum number of diffraction interactions in generated path candidates, avoiding combinatorial explosion when generating multi-order candidates with diffraction enabled (by <gh-user:jeertmans>, in <gh-pr:537>).
- Added `interaction_types` and `use_image_method` parameters to {func}`fermat_path_on_linear_objects<differt.geometry.fermat_path_on_linear_objects>`, forwarding interaction classifications to `fpt_jax.trace_rays` to inline the image method for planar specular reflections and accelerate Fermat edge-diffraction path solving. `interaction_types` now accepts {class}`InteractionType<differt.em.InteractionType>` members directly (raising via {func}`equinox.error_if` on unsupported ones) and is inferred automatically when left unset. Also added `max_steps`/`rtol`/`atol` to opt into `fpt_jax.trace_rays`'s adaptive while-loop solver, and raised the default `steps` from 5 to 15 for a safer millimeter-accuracy margin (by <gh-user:jeertmans>, in <gh-pr:537>).
- Added integration tests ({file}`differt/tests/integration/test_against_sionna_rt.py`) validating pure diffraction and mixed reflection-diffraction paths against Sionna RT on `simple_street_canyon` (by <gh-user:jeertmans>, in <gh-pr:537>).
...
- Reorganized integration tests into the {file}`differt/tests/integration/` subfolder, splitting `test_integration.py` into {file}`differt/tests/integration/test_against_sionna_rt.py` and {file}`differt/tests/integration/test_against_open3d.py` to use module-level `pytest.importorskip` checks (by <gh-user:jeertmans>, in <gh-pr:537>).
...
- Fixed `differt_core`'s ITU radio material XML parser, which always silently dropped the `<float name="thickness" value="..."/>` element (`Material.thickness` was always `None`, even when explicitly set in the scene file) (by <gh-user:jeertmans>, in <gh-pr:537>).
- Fixed a long-standing bug in `differt_core.geometry.Mesh.append` (used internally by {meth}`Scene.load_xml<differt.geometry.Scene.load_xml>`) that silently dropped the material name of the very first shape appended into an empty mesh, mislabeling its faces with whatever material the *second* appended shape happened to have; every other shape was assigned correctly. This affected every multi-object scene ever loaded via {meth}`Scene.load_xml<differt.geometry.Scene.load_xml>` (by <gh-user:jeertmans>, in <gh-pr:537>).
- Fixed {attr}`GeometricFieldSolver.radio_materials<differt.em.GeometricFieldSolver.radio_materials>` raising a `TypeError` as soon as a JIT-compiled function was called with a {class}`Material<differt.em.Material>` holding a `jax.Array` (rather than a plain Python scalar) in a non-static field. {meth}`GeometricFieldSolver.reflection_matrix<differt.em.GeometricFieldSolver.reflection_matrix>`, {meth}`diffraction_matrix<differt.em.GeometricFieldSolver.diffraction_matrix>`, {meth}`scattering_matrix<differt.em.GeometricFieldSolver.scattering_matrix>`, and {meth}`transmission_matrix<differt.em.GeometricFieldSolver.transmission_matrix>` now extract concrete per-material property arrays from {attr}`radio_materials<differt.em.GeometricFieldSolver.radio_materials>` eagerly, then delegate the actual (materials-independent) physics to an internally `jax.jit`-compiled core, so `radio_materials` never needs to cross a JIT boundary (nor be hashable) at all -- {class}`MaterialsDict<differt.em._material.MaterialsDict>` remains an ordinary, unhashable mapping (by <gh-user:jeertmans>, in <gh-pr:537>).
- Fixed a jaxtyping/beartype type-check failure in {class}`TracedPaths<differt.geometry.TracedPaths>`/{class}`LaunchedPaths<differt.geometry.LaunchedPaths>`'s `reshape`/`squeeze` methods, whose shared internal helpers incorrectly annotated the pre- and post-reshape/squeeze batch shapes with the same jaxtyping dimension name, even though these methods change that shape by design (by <gh-user:jeertmans>, in <gh-pr:537>).
- Fixed diffraction wavefront propagation edge curvature tracking in {meth}`WavefrontState.diffract<differt.em.WavefrontState.diffract>` when curvature along the edge plane is zero, eliminated NaN gradients under reverse-mode automatic differentiation in {func}`propagate_wavefront<differt.em.propagate_wavefront>` for planar wavefronts, and unified the UTD distance parameter $L$ calculation in {meth}`GeometricFieldSolver.diffraction_matrix<differt.em.GeometricFieldSolver.diffraction_matrix>` and {func}`L_i<differt.em.L_i>` using a safe curvature-based formulation adhering to McNamara et al. (1990) Chapter 6 (by <gh-user:jeertmans>, in <gh-pr:537>).
- Fixed {class}`HybridPathTracer<differt.geometry.HybridPathTracer>` and {class}`ExhaustivePathTracer<differt.geometry.ExhaustivePathTracer>` chunk candidate iterators crashing with ``ValueError: __len__() should return >= 0`` when the number of chunks is not known ahead of time, by returning an unsized iterator and allowing {class}`SizedIterator<differt.geometry.SizedIterator>` to represent iterators of unknown length without failing length hint checks (by <gh-user:jeertmans>, in <gh-pr:537>).
- Fixed {class}`GeometricFieldSolver<differt.em.GeometricFieldSolver>` type annotations for `tx_wavefront_radii` to accept {class}`WavefrontState<differt.em.WavefrontState>` instances directly in addition to tuples and scalars (by <gh-user:jeertmans>, in <gh-pr:537>).
- Fixed {class}`HybridPathTracer<differt.geometry.HybridPathTracer>` dropping non-reflection interaction types and misindexing visibility on {attr}`Mesh.assume_quads<differt.geometry.Mesh.assume_quads>` meshes, and added support for candidate chunk padding via ``pad_chunks`` (by <gh-user:jeertmans>, in <gh-pr:537>).
...
- **Breaking change**: Removed the deprecated `TriangleMesh` alias for {class}`Mesh<differt.geometry.Mesh>` (and its `differt_core.geometry` counterpart), which had been kept since v0.10.0 for backwards compatibility (by <gh-user:jeertmans>, in <gh-pr:537>).
- **Breaking change**: Removed the deprecated `TriangleScene` alias for {class}`Scene<differt.geometry.Scene>` (and its `differt_core.geometry` counterpart), which had been kept since v0.10.0 for backwards compatibility (by <gh-user:jeertmans>, in <gh-pr:537>).
- **Breaking change**: Removed the deprecated `Paths` and `SBRPaths` aliases for {class}`TracedPaths<differt.geometry.TracedPaths>` and {class}`LaunchedPaths<differt.geometry.LaunchedPaths>`, respectively, which had been kept since v0.10.0 for backwards compatibility (by <gh-user:jeertmans>, in <gh-pr:537>).
- **Breaking change**: Removed the deprecated `Scene.compute_paths` method, which had been kept since v0.10.0; use {meth}`Scene.trace_paths<differt.geometry.Scene.trace_paths>` or {meth}`Scene.launch_paths<differt.geometry.Scene.launch_paths>` instead (by <gh-user:jeertmans>, in <gh-pr:537>).

(changelog-v0.11.0)=
## [0.11.0](https://github.com/jeertmans/DiffeRT/compare/v0.10.0...v0.11.0)

(changelog-v0.11.0-added)=
### Added
...
- Added {class}`SBRPathTracer<differt.geometry.SBRPathTracer>`, a path tracer that *discovers* path candidates with a bounded, shooting-and-bouncing-rays (SBR) population instead of enumerating a (possibly visibility-pruned) complete graph, before solving them exactly with the same image-method solver used by {class}`ExhaustivePathTracer<differt.geometry.ExhaustivePathTracer>` and {class}`HybridPathTracer<differt.geometry.HybridPathTracer>`. Unlike those two, its candidate generation cost is bounded by `num_rays` and `max_num_candidates`, and does not grow combinatorially with the interaction order or the number of primitives in the scene, at the cost of no longer being exhaustive. Available via `solver="sbr"` in {meth}`Scene.trace_paths()<differt.geometry.Scene.trace_paths>`, and is now the default solver there, see the **Breaking change** entry below (by <gh-user:jeertmans>, in <gh-pr:529>).
- Added support for combining path candidates of different orders in a single call, by passing a sequence of orders — a list, a {class}`range`, or a `slice` with a defined `stop` (e.g., `order=[0, 1, 2]` or, equivalently, `order=range(0, 3)`) — to {meth}`AbstractPathTracer.generate_path_candidates()<differt.geometry.AbstractPathTracer.generate_path_candidates>`, {meth}`AbstractPathTracer.trace_paths()<differt.geometry.AbstractPathTracer.trace_paths>`, and {meth}`Scene.trace_paths()<differt.geometry.Scene.trace_paths>`. For {class}`ExhaustivePathTracer<differt.geometry.ExhaustivePathTracer>` and {class}`HybridPathTracer<differt.geometry.HybridPathTracer>`, candidates are generated independently for each order and directly combined into a single array of the correct, known-ahead-of-time size, with lower-order candidates padded with `-1` placeholders up to the maximum requested order; the combined array is then solved in a single call. Not currently compatible with a solver's `chunk_size` (by <gh-user:jeertmans>, in <gh-pr:529>).
- Every Warp kernel launch in the library (ray/triangle intersection, first-hit queries, visibility queries, {meth}`Scene.compute_tx_mlm()<differt.geometry.Scene.compute_tx_mlm>`, and {class}`SBRPathTracer<differt.geometry.SBRPathTracer>` candidate discovery) now automatically splits its work across CPU threads when running on a `"cpu"` Warp device, instead of running single-threaded. A single `wp.launch` call only ever uses one CPU core (see <ext-gh-issue:NVIDIA/warp#224>); this change manually dispatches the launch across multiple concurrent, disjoint-slice `wp.launch` calls instead. This is purely an internal performance change, with no effect on GPU execution or on any public API (by <gh-user:jeertmans>, in <gh-pr:529>).
- Added {func}`check_path_candidates<differt.geometry.check_path_candidates>`, which validates that `-1` placeholder values (marking an inactive/padded interaction, e.g., when combining path candidates of different orders) only ever appear as a trailing suffix of a path candidate. It is automatically called on every array of path candidates passed to {meth}`AbstractPathTracer.trace_path_candidates()<differt.geometry.AbstractPathTracer.trace_path_candidates>` (by <gh-user:jeertmans>, in <gh-pr:529>).
- {meth}`AbstractPathTracer.trace_path_candidates()<differt.geometry.AbstractPathTracer.trace_path_candidates>` (and the {class}`ExhaustivePathTracer<differt.geometry.ExhaustivePathTracer>`/{class}`HybridPathTracer<differt.geometry.HybridPathTracer>`/{class}`SBRPathTracer<differt.geometry.SBRPathTracer>` image-method solving it shares) now correctly supports path candidates padded with `-1` placeholders: a padded interaction resolves to the receiver's own position, reached right after the candidate's last genuine interaction, instead of being silently mistraced. This also applies to user-supplied path candidates passed via {meth}`Scene.trace_paths(path_candidates=...)<differt.geometry.Scene.trace_paths>`, which now correctly preserves `-1` placeholders (instead of rounding them down to `-2`) on {attr}`Mesh.assume_quads<differt.geometry.Mesh.assume_quads>` meshes, and reports them as inactive (`-1`), not specular (`0`), in the resulting `interaction_types` (by <gh-user:jeertmans>, in <gh-pr:529>).
...
- All file reader methods now support {class}`os.PathLike[str]<os.PathLike>` in addition to {class}`str` input types (by <gh-user:jeertmans>, in <gh-pr:517>).
- **Breaking change**: The default `solver` of {meth}`Scene.trace_paths()<differt.geometry.Scene.trace_paths>` changed from `"exhaustive"` to `"sbr"` (i.e., {class}`SBRPathTracer<differt.geometry.SBRPathTracer>`). Calling `trace_paths` without an explicit `solver` now scales much better with scene size and interaction order, but is **no longer guaranteed to find every valid path**; pass `solver="exhaustive"` (or `"hybrid"`) explicitly to restore the previous, deterministic-and-exhaustive behavior. As part of this change, a solver is also no longer required to be given an `order` when `path_candidates` is provided explicitly, even for {class}`HybridPathTracer<differt.geometry.HybridPathTracer>`-family solvers (by <gh-user:jeertmans>, in <gh-pr:529>).
- Refactored {func}`deepmimo.export<differt.plugins.deepmimo.export>` to leverage the new unified JAX-compatible `compute_received_fields` pipeline, reducing duplicate code and improving consistency (by <gh-user:jeertmans>, in <gh-pr:460>).
...
(changelog-v0.11.0-fixed)=
### Fixed

- Passing a negative `order` (or a sequence/`range`/`slice` containing one) to a path solver's `generate_path_candidates()`, `trace_paths()`, or {meth}`Scene.trace_paths()<differt.geometry.Scene.trace_paths>` now raises a `ValueError` immediately, instead of silently producing meaningless results (by <gh-user:jeertmans>, in <gh-pr:529>).
- Fixed a crash (`CUDA_ERROR_NOT_SUPPORTED: Failed to create a child graph node`) affecting every Warp-accelerated method (used by all path tracers, {meth}`Mesh.first_triangle_hit_by_ray()<differt.geometry.Mesh.first_triangle_hit_by_ray>`, {meth}`Mesh.ray_intersect_any_triangle()<differt.geometry.Mesh.ray_intersect_any_triangle>`, {meth}`Mesh.triangles_visible_from_vertex()<differt.geometry.Mesh.triangles_visible_from_vertex>`, and {meth}`Scene.compute_tx_mlm()<differt.geometry.Scene.compute_tx_mlm>`) when running on a GPU under some virtualized CUDA setups (e.g., WSL2), where these environments don't support nesting a Warp kernel launch as a "child" node inside a CUDA graph that JAX/XLA builds around the surrounding compiled computation. Every `wp.jax_callable` call now explicitly opts out of that graph capture (`graph_mode=warp.JaxCallableGraphMode.NONE`), which has no effect on CPU execution and, if anything, measurably speeds up GPU execution too (by <gh-user:jeertmans>, in <gh-pr:529>).
- Fixed a Warp mesh cache collision issue where reusing Python object memory addresses (`id(mesh)`) could cause {class}`SBRPathTracer<differt.geometry.SBRPathTracer>` or {class}`Mesh<differt.geometry.Mesh>` ray-query methods to hit stale cached meshes with different vertices or masks (by <gh-user:jeertmans>, in <gh-pr:529>).
- Fixed {class}`SBRPathTracer<differt.geometry.SBRPathTracer>` candidate generation padding up to `max_num_candidates` with placeholder `-1` rows, which caused huge memory allocations and (especially) spurious duplicate line-of-sight paths downstream, and updated multi-order generation to extract genuine trajectory prefixes at each requested bounce order matching reference Sionna RT behavior (by <gh-user:jeertmans>, in <gh-pr:460>).

(changelog-v0.10.0)=
## [0.10.0](https://github.com/jeertmans/DiffeRT/compare/v0.9.1...v0.10.0)

(changelog-v0.10.0-added)=
### Added
...
- **Breaking change**: Renamed multiple functions and methods across the codebase from plural to singular form to improve API consistency and better align with JAX broadcasting semantics:
  - **Ray tracing (`differt.rt`):**
    - Renamed `rays_intersect_any_triangle` to {func}`ray_intersect_any_triangle<differt.geometry.ray_intersect_any_triangle>`
    - Renamed `first_triangles_hit_by_rays` to {func}`first_triangle_hit_by_ray<differt.geometry.first_triangle_hit_by_ray>`
    - Renamed `triangles_visible_from_vertices` to {func}`triangles_visible_from_vertex<differt.geometry.triangles_visible_from_vertex>`
    - Renamed `rays_intersect_triangles` to {func}`ray_intersect_triangle<differt.geometry.ray_intersect_triangle>`
    - Renamed `consecutive_vertices_are_on_same_side_of_mirrors` to {func}`consecutive_vertices_are_on_same_side_of_mirror<differt.geometry.consecutive_vertices_are_on_same_side_of_mirror>`
    - Renamed `image_of_vertices_with_respect_to_mirrors` to {func}`image_of_vertex_with_respect_to_mirror<differt.geometry.image_of_vertex_with_respect_to_mirror>`
    - Renamed `intersection_of_rays_with_planes` to {func}`intersection_of_ray_with_plane<differt.geometry.intersection_of_ray_with_plane>`
...
- Added a generic type variable for `Paths.mask` (by <gh-user:jeertmans>, in <gh-pr:349>).

(changelog-v0.7.0-fixed)=
### Fixed
...
(changelog-v0.6.2-changed)=
### Changed

- Changed {func}`fermat_path_on_linear_objects<differt.geometry.fermat_path_on_linear_objects>` to leverage the `fpt-jax` library {cite}`fpt-eucap2026` for better performance and faster gradient computations (by <gh-user:jeertmans>, in <gh-pr:335>).

(changelog-v0.6.2-chore)=
### Chore
...
- Added a `batch_size` argument to `TriangleScene.compute_paths` to allow users to specify the size of the batch used for ray-triangle intersection tests, see [above](#ray-triangle-batch-size-none) (by <gh-user:jeertmans>, in <gh-pr:310>).
- Added the {meth}`DiGraph.filter_by_mask<differt_core.geometry.DiGraph.filter_by_mask>` to disconnected nodes based on a mask (by <gh-user:jeertmans>, in <gh-pr:322>).
- Added a `disconnect_inactive_triangles` to `TriangleScene.compute_paths` to allow reducing the number of path candidates, at the cost of potential recompilations (by <gh-user:jeertmans>, in <gh-pr:322>).
...
(changelog-v0.6.0-changed)=
### Changed

- Changed {meth}`DiGraph.disconnect_nodes<differt_core.geometry.DiGraph.disconnect_nodes>` to raise an {class}`IndexError` when the node indices are out of bounds (by <gh-user:jeertmans>, in <gh-pr:322>).
...
(changelog-v0.5.0-fixed)=
### Fixed

- Fixed a shape (and possibly `dtype`) issue in the fast path of `consecutive_vertices_are_on_same_side_of_mirrors`, that would raise an error when trying to stack arrays in `TriangleScene.compute_paths` with a non-{data}`None` value for `smoothing_factor` (by <gh-user:jeertmans>, in <gh-pr:303>).

(changelog-v0.5.0-removed)=
### Removed

- **Breaking change**: Removed `parallel` keyword argument in `TriangleScene.compute_paths` as it was no longer supported, and its presence increased the code complexity. Executing code on multiple devices should be automatically handled by {func}`jax.jit`, or manually specified by the end-user (by <gh-user:jeertmans>, in <gh-pr:305>).

(changelog-v0.4.1)=
## [0.4.1](https://github.com/jeertmans/DiffeRT/compare/v0.4.0...v0.4.1)

(changelog-v0.4.1-added)=
### Added
...
(changelog-v0.4.1-chore)=
### Chore

- Refactored {func}`image_method<differt.geometry.image_method>` and {func}`fermat_path_on_linear_objects<differt.geometry.fermat_path_on_linear_objects>` to use {func}`jnp.vectorize<jax.numpy.vectorize>` instead of a custom but complex chain of calls to {func}`jax.vmap`, reducing the code complexity while not affecting performance (by <gh-user:jeertmans>, in <gh-pr:298>).
...
(changelog-v0.3.1-added)=
### Added

- Implemented `method = 'hybrid'` for `TriangleScene.compute_paths` (by <gh-user:jeertmans>, in <gh-pr:295>).

(changelog-v0.3.1-chore)=
### Chore

- Reduced computational time of higher-order RT tests by leveraging visibility matrices (by <gh-user:jeertmans>, in <gh-pr:294>).

(changelog-v0.3.1-fixed)=
### Fixed
...
- Changed the default `optimizer` used by {func}`fermat_path_on_linear_objects<differt.geometry.fermat_path_on_linear_objects>` to be {func}`optax.lbfgs` (by <gh-user:jeertmans>, in <gh-pr:272>).

(changelog-v0.1.2-fixed)=
### Fixed

- Fixed {class}`ValueError` raised when using `parallel` mode in `TriangleScene.compute_paths` with `jax>=0.6` by disabling it (see <gh-issue:280>). This is a *soft* **breaking change** as it will raise a warning (by <gh-user:jeertmans>, in <gh-pr:281>). Using JAX v0.6 (and above) is now allowed again.

(changelog-v0.1.1)=
## [0.1.1](https://github.com/jeertmans/DiffeRT/compare/v0.1.0...v0.1.1)

(changelog-v0.1.1-added)=
### Added
...
- Added {meth}`from_mitsuba<differt.geometry.Scene.from_mitsuba>` and {meth}`from_sionna<differt.geometry.Scene.from_sionna>` methods to the {class}`TriangleScene<differt.geometry.Scene>` class (by <gh-user:jeertmans>, in <gh-pr:267>).

(changelog-v0.1.1-chore)=
### Chore
...
(changelog-v0.1.1-fixed)=
### Fixed

- Fixed potential {class}`IndexError` in {attr}`TriangleScene.num_{transmitters,receivers}<differt.geometry.Scene.num_transmitters>` when the TX/RX arrays have incorrect shape (by <gh-user:jeertmans>, in <gh-pr:272>).
