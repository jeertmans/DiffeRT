...
- **Breaking change**: {meth}`Scene.trace_paths<differt.geometry.Scene.trace_paths>`, {meth}`Scene.trace_fields<differt.geometry.Scene.trace_fields>`, and {meth}`AbstractPathTracer.generate_path_candidates<differt.geometry.AbstractPathTracer.generate_path_candidates>` now accept an `allowed_interactions` keyword argument (a `frozenset` of {class}`InteractionType<differt.em.InteractionType>`, e.g. `frozenset({SpecularReflection, Diffraction})`), replacing (and removing) the previous, effectively-unreachable `specular_reflection`/`diffuse_scattering` booleans. {class}`ExhaustivePathTracer<differt.geometry.ExhaustivePathTracer>`, {class}`HybridPathTracer<differt.geometry.HybridPathTracer>`, and {class}`SBRPathTracer<differt.geometry.SBRPathTracer>` can now generate and trace ``DIFFRACTION`` (via {func}`fermat_path_on_linear_objects<differt.geometry.fermat_path_on_linear_objects>`, mixing reflection/scattering planes and diffraction edges in one joint solve) and ``TRANSMISSION`` (which does not bend the ray: its position is spliced in afterward, as the intersection of the straight segment between its two nearest surrounding "real" vertices -- solved bends, or the transmitter/receiver -- with its transmissive face) bounces, in addition to today's ``REFLECTION``/``SCATTERING``. Defaults to reflection-only, matching prior behavior exactly (by <gh-user:jeertmans>, in <gh-pr:537>).
...
- Added a per-call `wavefront_radii` keyword argument to {func}`compute_received_fields<differt.em.compute_received_fields>`, {meth}`GeometricFieldSolver.compute_fields<differt.em.GeometricFieldSolver.compute_fields>`, and {meth}`GeometricFieldSolver.spreading_factor<differt.em.GeometricFieldSolver.spreading_factor>`, letting the incident wavefront be overridden for a single call, even when a pre-built solver instance is passed (the one exception to `solver_kwargs` otherwise being disallowed together with a solver instance) (by <gh-user:jeertmans>, in <gh-pr:537>).
...
- Fixed `differt_core`'s ITU radio material XML parser, which always silently dropped the `<float name="thickness" value="..."/>` element (`Material.thickness` was always `None`, even when explicitly set in the scene file). Now that thickness is actually parsed, {meth}`Scene.load_xml<differt.geometry.Scene.load_xml>` also keys two same-named ITU materials by their raw XML id, instead of merging them under one shared name, whenever they disagree on thickness (by <gh-user:jeertmans>, in <gh-pr:537>).
...
- Fixed the UTD distance parameter $L$ calculation in {meth}`GeometricFieldSolver.diffraction_matrix<differt.em.GeometricFieldSolver.diffraction_matrix>` and {func}`L_i<differt.em.L_i>`, which divided directly by the edge-fixed radius of curvature $\rho_e^i$ and could produce `NaN` (including under reverse-mode automatic differentiation) at that limit; both now go through a safe, curvature-reciprocal-based formulation adhering to McNamara et al. (1990) Chapter 6 (by <gh-user:jeertmans>, in <gh-pr:537>).
...
- **Breaking change**: Removed the `differt.em.transition_matrices` module-level alias for {func}`transition_matrix<differt.em.transition_matrix>`; use {func}`transition_matrix<differt.em.transition_matrix>` directly instead (by <gh-user:jeertmans>, in <gh-pr:537>).
