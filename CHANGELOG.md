...
- Added an extensible EM field-solver architecture ({class}`AbstractFieldSolver<differt.em.AbstractFieldSolver>`, {class}`GeometricFieldSolver<differt.em.GeometricFieldSolver>`) and a reusable, JAX-compatible pipeline ({func}`compute_received_fields<differt.em.compute_received_fields>`, {func}`compute_received_power<differt.em.compute_received_power>`, {func}`compute_cir<differt.em.compute_cir>`) to compute received fields, power, and Channel Impulse Response (CIR), supporting reflection, {meth}`transmission<differt.em.GeometricFieldSolver.transmission_matrix>` (finite-thickness dielectric slab, per ITU-R P.2040-3), {func}`diffraction<differt.em.diffraction_coefficients>` (Uniform Theory of Diffraction, Luebbers' heuristic model for PEC and lossy dielectric wedges), and {meth}`diffuse scattering<differt.em.GeometricFieldSolver.scattering_matrix>` (Degli-Esposti's Lambertian model) interactions, plus a `tx_wavefront_radii` attribute for non-planar (near-field) sources; custom interaction models can be added by subclassing and overriding a `*_matrix` method. Solver-specific configuration (`tx_polarization`, `rx_polarization`, `radio_materials`, `tx_wavefront_radii`) lives on the {class}`GeometricFieldSolver<differt.em.GeometricFieldSolver>` instance itself, mirroring {class}`ExhaustivePathTracer<differt.geometry.ExhaustivePathTracer>`; {func}`compute_received_fields<differt.em.compute_received_fields>` accepts either a `solver="geometric"` string shortcut (with those same attributes passed as keyword arguments, instantiating a {class}`GeometricFieldSolver<differt.em.GeometricFieldSolver>` under the hood) or a solver instance directly (by <gh-user:jeertmans>, in <gh-pr:460>).
- Added {attr}`InteractionType.TRANSMISSION<differt.em.InteractionType.TRANSMISSION>` and new {attr}`Material.scattering_coefficient<differt.em._material.Material.scattering_coefficient>` / {attr}`Material.xpd_coefficient<differt.em._material.Material.xpd_coefficient>` fields (both defaulting to `0.0`, so existing materials are unaffected) (by <gh-user:jeertmans>, in <gh-pr:460>).
- Generalized {attr}`GeometricFieldSolver.tx_wavefront_radii<differt.em.GeometricFieldSolver.tx_wavefront_radii>` to accept a `(rho_s, rho_p)` tuple of two independent principal radii of curvature, for a general **astigmatic** (as opposed to spherical) non-planar source, or `None`, for a **planar** wavefront (the far-field, plane-wave approximation, e.g. via {class}`AbstractFarFieldAntenna<differt.em.AbstractFarFieldAntenna>`); a single value still defaults to an isotropic (spherical) wavefront. An astigmatic source (unlike a planar one) is not yet supported together with a `DIFFRACTION` interaction, which would require tracking the wavefront's principal-axis orientation along the path (by <gh-user:jeertmans>, in <gh-pr:460>).
- Added {meth}`AbstractAntenna.wavefront_radii<differt.em.AbstractAntenna.wavefront_radii>`, letting each antenna report the radius (or radii) of curvature of the wavefront it emits, in a given propagation direction, as either a scalar (isotropic/spherical), a `(rho_s, s_hat, rho_p, p_hat)` 4-tuple (astigmatic, with the local s-/p-plane orientation), or `None` (planar); {meth}`GeometricFieldSolver.compute_fields<differt.em.GeometricFieldSolver.compute_fields>` uses it (overriding {attr}`~differt.em.GeometricFieldSolver.tx_wavefront_radii`) whenever {attr}`~differt.em.GeometricFieldSolver.tx_polarization` is an {class}`AbstractAntenna<differt.em.AbstractAntenna>` instance. {class}`Dipole<differt.em.Dipole>` implements it as an ideal point source (a zero offset in every direction). Added {class}`AbstractFarFieldAntenna<differt.em.AbstractFarFieldAntenna>`, a base class for antennas only ever used in the far field (`wavefront_radii` unconditionally returns `None`, i.e., a planar wavefront), and {class}`FarFieldDipoleAntenna<differt.em.FarFieldDipoleAntenna>`, the corresponding far-field-only variant of {class}`Dipole<differt.em.Dipole>` (by <gh-user:jeertmans>, in <gh-pr:460>).
- {func}`compute_received_fields<differt.em.compute_received_fields>` now derives `frequency` automatically from `tx_polarization`'s {attr}`~differt.em.AbstractAntenna.frequency` when `tx_polarization` is an {class}`AbstractAntenna<differt.em.AbstractAntenna>` instance and `frequency` is left unspecified, letting a single antenna model (or one antenna model per transmitter, via batched array fields) drive both the radiation pattern and the operating frequency (by <gh-user:jeertmans>, in <gh-pr:460>).
- Added a new {attr}`Material.scattering_pattern<differt.em._material.Material.scattering_pattern>` field, letting each material define its own diffuse scattering pattern, rather than always using the built-in Lambertian one; custom patterns can be added by subclassing the new {class}`AbstractScatteringPattern<differt.em._material.AbstractScatteringPattern>` abstract class and implementing its `__call__` method. Defaults to the new {class}`LambertianPattern<differt.em._material.LambertianPattern>`, so existing materials are unaffected (by <gh-user:jeertmans>).
- Added {class}`DirectivePattern<differt.em._material.DirectivePattern>` and {class}`BackscatteringPattern<differt.em._material.BackscatteringPattern>`, two additional {class}`AbstractScatteringPattern<differt.em._material.AbstractScatteringPattern>` implementations following Degli-Esposti's directive/combined scattering models: the former concentrates diffusely-scattered power in a lobe around the specular direction (width controlled by `alpha_r`), while the latter mixes that lobe with a second one around the retroreflection direction (width `alpha_i`), weighted by `lambda_` (by <gh-user:jeertmans>).
...
- Removed the broken, unimplemented `differt.em.transition_matrix` stub, superseded by {meth}`GeometricFieldSolver.transition_matrices<differt.em.GeometricFieldSolver.transition_matrices>` (by <gh-user:jeertmans>).
...
- Reworked the {doc}`quickstart notebook <notebooks/quickstart>`'s interaction-type examples (reflection, diffraction, transmission, diffuse scattering) to plot received power as the receiver moves, rather than printing single dBW values, illustrating e.g. two-ray interference and the UTD shadow region (matching the region/boundary structure of [Sionna RT's own diffraction tutorial](https://nvlabs.github.io/sionna/rt/tutorials/Diffraction.html)), each alongside a 3D Plotly figure of the corresponding scenario (by <gh-user:jeertmans>, in <gh-pr:460>).

### Fixed

- Fixed {func}`diffraction_coefficients<differt.em.diffraction_coefficients>` using the incidence angle instead of the diffraction angle when computing the n-face reflection coefficient (`cos_theta_n`); only affected non-PEC (`n_r_n` provided) wedge faces (by <gh-user:jeertmans>, in <gh-pr:460>).
- Fixed the `mode == "-"` branch in the UTD `_N` helper function in `differt/em/_utd.py` to use `(beta - jnp.pi)` instead of `(beta + jnp.pi)` (by <gh-user:jeertmans>, in <gh-pr:460>).
- Fixed {meth}`GeometricFieldSolver.diffraction_matrix<differt.em.GeometricFieldSolver.diffraction_matrix>` clipping a wedge face's "no thickness specified" sentinel to `0.0` (a genuinely zero-thickness, and thus nearly transparent, slab) instead of falling back to the infinite-half-space Fresnel reflection coefficient, like {meth}`GeometricFieldSolver.reflection_matrix<differt.em.GeometricFieldSolver.reflection_matrix>` does for the same sentinel; this broke the continuity of the total field across the reflection shadow boundary (RSB) for any wedge face material without an explicit `thickness` (i.e., the common case) (by <gh-user:jeertmans>, in <gh-pr:460>).
- Fixed an extraneous factor of $\sqrt{2}$ in the UTD `_cot_times_F` helper function's shadow-boundary limit value (used only within `1e-5` rad of an exact incident/reflection shadow boundary), derived from an incorrect small-argument asymptotic of the transition function {func}`F<differt.em.F>`; this broke the continuity of the total field exactly at the incident shadow boundary (ISB) (by <gh-user:jeertmans>, in <gh-pr:460>).
...
- Added a `batch_size` argument to {meth}`TriangleScene.compute_paths<differt.geometry.Scene.compute_paths>` to allow users to specify the size of the batch used for ray-triangle intersection tests, see [above](#ray-triangle-batch-size-none) (by <gh-user:jeertmans>, in <gh-pr:310>).
...
- Added a `disconnect_inactive_triangles` to {meth}`TriangleScene.compute_paths<differt.geometry.Scene.compute_paths>` to allow reducing the number of path candidates, at the cost of potential recompilations (by <gh-user:jeertmans>, in <gh-pr:322>).
...
- Fixed a shape (and possibly `dtype`) issue in the fast path of `consecutive_vertices_are_on_same_side_of_mirrors`, that would raise an error when trying to stack arrays in {meth}`TriangleScene.compute_paths<differt.geometry.Scene.compute_paths>` with a non-{data}`None` value for `smoothing_factor` (by <gh-user:jeertmans>, in <gh-pr:303>).
...
- Implemented `method = 'hybrid'` for {meth}`TriangleScene.compute_paths<differt.geometry.Scene.compute_paths>` (by <gh-user:jeertmans>, in <gh-pr:295>).
...
- Fixed {class}`ValueError` raised when using `parallel` mode in {meth}`TriangleScene.compute_paths<differt.geometry.Scene.compute_paths>` with `jax>=0.6` by disabling it (see <gh-issue:280>). This is a *soft* **breaking change** as it will raise a warning (by <gh-user:jeertmans>, in <gh-pr:281>). Using JAX v0.6 (and above) is now allowed again.
